...
import {
  RecentlyViewedEventItem,
  useScreenContext,
} from "@/views/recently-viewed-events/context/recently-viewed-events-screen-context";
import { DateTime } from "luxon";

function mapEventToRecentlyViewedEventItem(
  event: Event,
): RecentlyViewedEventItem {
...
export function useOperation() {
  ...
  const loadEvents = useCallback(async () => {
    try {
      setIsLoading(true);
      const result = await EventService.getViewHistory(null, 20);
      setEvents(result.events.map(mapEventToRecentlyViewedEventItem));
  ...
  const loadNextPage = useCallback(async () => {
    if (!nextToken) return;
    try {
      const result = await EventService.getViewHistory(nextToken, 20);
      setEvents([
        ...events,
        ...result.events.map(mapEventToRecentlyViewedEventItem),
      ]);
      setNextToken(result.nextToken);
    } catch (e) {
      throw e;
